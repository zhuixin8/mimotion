...
test('avatar follows saved plan and fresh task state without claiming successful synchronization',()=>{
 ...
 assert.equal(avatarState(account,runtime,now,now).motion,'idle');
 ...
 assert.equal(avatarState(account,{latest:{status:'queued'}},now,now).motion,'idle');
...
test('avatar static resources are local, cacheable and do not require DB access; APIs remain uncached',async()=>{
 ...
 const js=await get('/athlete-v2.js');assert.equal(js.status,200);assert.match(js.headers.get('cache-control'),/immutable/);assert.match(js.headers.get('content-type'),/javascript/);assert.ok(!(await js.text()).includes('esm.sh/'));
