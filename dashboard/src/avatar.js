...
export function createAvatar() {
  ...
  card.innerHTML='<div class="athlete-stage"><span class="athlete-fallback">人物加载中…</span></div>';
  const stage=card.querySelector('.athlete-stage'),fallback=card.querySelector('.athlete-fallback');
  ...
  function restart(){cancel();draw();if(canDraw()&&!reduced.matches&&presentation.animate)frame=requestAnimationFrame(animate);}
  function apply(){
    ...
    card.dataset.motion=p.motion;
    const next=actions[p.motion];
    if(next&&next!==active){next.reset().setEffectiveTimeScale(1).setEffectiveWeight(1).play();if(active)next.crossFadeFrom(active,.45,true);active=next;if(reduced.matches||!p.animate)mixer.update(.5);}
    if(changed||!frame)restart();
  }
  const visibility=()=>restart();document.addEventListener('visibilitychange',visibility);
  const motionPreference=()=>{if(loaded)mixer.update(.5);restart();};reduced.addEventListener('change',motionPreference);
  ...
  async function initialize(){
    try{
      ...
      renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();failed=true;loaded=false;cancel();fallback.hidden=false;fallback.textContent='动画暂不可用，计划正常运行';});
    ...
    }catch{failed=true;cancel();fallback.hidden=false;fallback.textContent='动画暂不可用，计划正常运行';renderer?.dispose();}
  }
