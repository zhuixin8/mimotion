...
export async function enqueue(env, account, kind = 'manual', ms = Date.now()) {
  const day = beijing(ms).slice(0, 10), t = seconds();
  const slot = kind === 'schedule' ? `schedule:${beijing(ms).slice(0, 13)}` : `manual:${Math.floor(t / 60)}`;
  ...
  const step = Math.max(previous?.step || 0, targetSteps(account.min_step, account.max_step, ms));
  await query(env, `INSERT INTO runs(id,account_id,slot,kind,day,step,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(account_id,slot) DO NOTHING`, crypto.randomUUID(), account.id, slot, kind, day, step, t, t).run();
...
export async function consume(message, env) {
  ...
  const finish = async (status, text) => query(env, 'UPDATE runs SET status=?,message=?,updated_at=? WHERE id=?', status, text, seconds(), run.id).run();
  if (run.day !== beijing().slice(0, 10)) { await finish('skipped', '任务已过期，未提交'); message.ack(); return; }
  ...
  try {
    ...
    const claim = await query(env, "UPDATE runs SET status='running',message='正在提交',updated_at=? WHERE id=? AND status IN ('queued','pending')", seconds(), run.id).run();
    ...
    run.step = Math.max(run.step, prior?.step || 0);
    ...
    await finish('failed', e instanceof UserError ? e.message : '执行失败，请稍后重试');
  } finally {
